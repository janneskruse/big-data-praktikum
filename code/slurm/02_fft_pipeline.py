...
def channel_wavelet_fcwt(data, args, positions, channel):
    ...
    Wsegs = np.zeros((n_segments, channel, num_frequency_points, seg_len), dtype=fft_dtype)
...
def channel_fourier(data, args, taper, positions, channel):
    ...
    for i in range(n_segments):
        for channel_number, channel in enumerate(segments[i]):
            ...
            fourier_transformed = (10 * np.log(np.abs(fft_output) ** 2))[:num_frequency_points]  # Compute power spectrum
    ...
    xr_zarr = xr.Dataset(
        {
            ...
            "cwt": (["time", "channel", "frequency"], cwt_dask_array),
        },
        coords={
            "time": time_coords,
...
if __name__=='__main__':
    ...
    for i, liste in enumerate(split_up):
        ...
        fft_time=end-start
        parallel_runs=1
        if len(liste)>n_cores:
            parallel_runs=len(liste)//n_cores
        time_per_file=fft_time/parallel_runs
        ...
        # Concatanate xarrays
        print("Concatenating xarrays...")
        start=time.time()
        xr_zarr = xr.concat(fft_results, dim="time")
        print(f"Concatenated xarrays in {time.time()-start}s")
